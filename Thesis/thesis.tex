...
\begin{enumerate}
    \item \emph{Uncreative} | All music in the arrangement must come from the original, in the same order.
...
Reduces to maximum independent set problem when $n=1$.
...
Assuming we can calculate a quantity that provides a fairly objective judgement of the ``musicality'' of a phrase, we can aim to maximise this quantity to ensure the final arrangement is as interesting as possible.
...
The weight of each node is taken here to be the entropy of the phrase \cite{li_entropy_2019}. A similar idea of phrase `utility' has been used before \cite{huang_towards_2012} in order to assess playability rules.
