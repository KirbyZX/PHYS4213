...
def createBQM(G: nx.Graph, phrases: list, instruments: list, nodeMultiplier: float = 1, edgeMultiplier: float = 1, edgeWeightMultiplier: float = 1) -> BinaryQuadraticModel:
    ...
    for phrase in allPhrases:
        ...
        bqm.add_linear_inequality_constraint([(f"{phrase.id}_{i}", 1) for i in instruments.keys()], ub=1, lagrange_multiplier=nodeMultiplier*maxEntropy, penalization_method="slack", label="One colour per vertex")
    ...
    for u, v, d in G.edges.data():
        ...
        bqm.add_quadratic_from([(f"{u}_{i}", f"{v}_{i}", edgeMultiplier*maxEntropy) for i in instruments.keys()])
        # Maximise edge weighting
        bqm.add_quadratic_from([(f"{u}_{i}", f"{v}_{j}", -edgeWeightMultiplier*d["weight"]) for i,j in iter.combinations(instruments.keys(), 2)])
